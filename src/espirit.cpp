...
Cx4 ESPIRIT(Gridder const &gridder, Cx3 const &data, long const kRad, long const calRad, Log &log)
{
  ...
  Cx5 const mini_kernels = LowRank(all_mini_kernels, retain, log);
  Cx5 lo_kernels(
      lo_grid.dimension(0),
      lo_grid.dimension(1),
      lo_grid.dimension(2),
  ...
  auto cov_task = [&](long const lo_z, long const hi_z) {
    for (long zz = lo_z; zz < hi_z; zz++) {
      for (long yy = 0; yy < lo_kernels.dimension(2); yy++) {
        for (long xx = 0; xx < lo_kernels.dimension(1); xx++) {
          Cx2 const vox_cov = Covariance(lo_kernels.chip(zz, 3).chip(yy, 2).chip(xx, 1));
          // cov.chip(zz, 3).chip(yy, 2).chip(xx, 1) = vox_cov.reshape(Sz1{nchan * nchan});
          Cx2 vecs(vox_cov.dimensions());
          R1 vals(vox_cov.dimension(0));
          PCA(vox_cov, vecs, vals, log);
          Cx1 const vec0 = vecs.chip(nchan - 1, 1).conjugate();
          float const phase = std::arg(Cx0(vec0.mean())());
          vec.chip(zz, 3).chip(yy, 2).chip(xx, 1) = vec0 * std::polar(1.f, -phase);
