    ...
    Log &log)
{
  ...
  log.info(FMT_STRING("{} values of B1 from {} to {}"), nB1, B1Lo, B1Hi);
  long totalN = nT1 * nbeta * nB1;
  ...
  for (long iB1 = 0; iB1 < nB1; iB1++) {
    float const fiB1 = iB1 / (nB1 - 1.f);
    float const B1 = B1Lo + (B1Hi - B1Lo) * fiB1;
    ...
    for (long it = 0; it < nT1; it++) {
      ...
      for (long ib = 0; ib < nbeta; ib++) {
        ...
        // Now fill in dynamic
        long const row = (iB1 * nT1 + it) * nbeta + ib;
